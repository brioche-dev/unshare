...
impl Command {
    ...
    /// Disables setgroups syscall in user namespace by writing to
    /// `/proc/[pid]/setgroups`.
    ///
    /// This automatically enables `User` namespace. You should alo use
    /// `set_id_maps` and `uid`/`gid` to configure the user namespace for the
    /// new process.
    ///
    /// When `deny` is set to true, the setgroups(2) syscall will be permanently
    /// disabled in the user namespace. In doing so, this also allows `gid`
    /// to be used even if the parent process is unprivileged.
    pub fn deny_setgroups(&mut self, deny: bool) -> &mut Command {
        self.unshare(&[Namespace::User]);
        self.config.deny_setgroups = deny;
        self
    }
